...
#path for CSV fave
filepath = "E:\Kia_Wee_Work/For_Tracking/070720_RPE-mCh-LAMP1_Dex10kDa-AF488_WGA647_source/focus"
...
for current_file in files:
    ...
    for i, roi in enumerate(RoiManager.getInstance().getRoisAsArray()):
        ...
        stats = imp_measure.getStatistics(Measurements.MEAN | Measurements.AREA | Measurements.INTEGRATED_DENSITY | Measurements.FERET | Measurements.CENTROID)
        intden = stats.umean*stats.pixelCount

        ort.incrementCounter()
        #ort.addValue("ROI", str(i))
        ort.addValue("Mean intensity", str(stats.mean))
        ort.addValue("Area", str(stats.area))
        ort.addValue("Integrated density", str(intden))
    ...
    savename = "E:\Kia_Wee_Work/For_Tracking/070720_RPE-mCh-LAMP1_Dex10kDa-AF488_WGA647_source/"+filename
    ort.saveAs(savename)
    imp1.close()
